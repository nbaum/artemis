require 'artemis/protocol_types'
require 'stringio'
...
module Artemis
  ...
  module Encoder
    ...
    def self.encode (name, type, subtypes = [], &block)
      type &&= type.to_s(16)
      subtypes = [subtypes].flatten
      if !subtypes.empty?
        subtypes.each do |subtype|
          subtype &&= subtype.to_s(16)
          define_method "encode_#{type}_#{subtype}" do |io|
            {type: name}.tap{|h|instance_exec(io, h, &block)}
          end
        end
      else
        define_method "parse_#{type}" do |data|
          io = StringIO.new(data)
          io.extend ProtocolTypes
    ...
    def parse_subtype (type, subtype, io, h)
      type = type.to_s(16)
      subtype = subtype.to_s(16)
      __send__("parse_#{type}_#{subtype}", io, h)
    end
    ...
    decode :game_over_reason, 0xf754c8fe, 0x06 do |io, h|
      h[:reason] = []
      until io.eof?
    ...
    decode :object_update, 0x80803df9 do |io, h|
      ...
      until io.eof?
        type = io.read_byte
        break if type == 0
        updates << self.__send__("parse_80803df9_#{type.to_s(16)}", io)
      end
    ...
    #decode_subtypes 0x80803df9, :read_byte
    ...
    decode :eng_player_update, 0x80803df9, 0x03 do |io, h|
      ...
      %i"beams torpedos sensors maneuvering impulse ftl fore_shields aft_shields".each do |key|
        h[key] = {
          heat: fields.next(:float),
          energy: fields.next(:float),
          coolant: fields.next(:byte)
        }
        fields.pop
      end
    end
    ...
    decode :generic_update, 0x80803df9, [0x06, 0x07, 0x0a, 0x0b, 0x0c, 0x0e] do |io, h|
      h[:id] = io.read_int
    ...
    decode :main_player_update, 0x80803df9, 0x01 do |io, h|
      h[:id] = io.read_int
    ...
    decode :console_status, 0x19c6e2d4 do |io, h|
      h[:ship] = io.read_int
      h[:status] = {
        main_screen: io.read_byte,
        helm:        io.read_byte,
        weapons:     io.read_byte,
        engineering: io.read_byte,
        science:     io.read_byte,
        comms:       io.read_byte,
        data:        io.read_byte,
        observer:    io.read_byte,
        captain:     io.read_byte,
        game_master: io.read_byte,
      }
    end
